...
var RB_PIXEL_URL = "https://ad.mail.ru/i1556.gif";
...
function fetchWithCredentials(url, onStateChange) {
	if (browser == Browsers.FIREFOX) {
...
Metric.prototype.send = function() {
	...
	var self = this;
	fetchWithCredentials(this.getUrl(), function(xhr) {
		self.sending = false;
...
if (browser == Browsers.CHROME) {
	onStart = function() {
		MetricsInterface.sendIfNecessary();
		if (RB_PIXEL_URL) {
			fetchWithCredentials(RB_PIXEL_URL, function() {});
		}
